...
describe("recurring-issues", () => {
	...
	function runBot({ issueTitle, labelNames, eventTypes }) {
		...
		fetchMock.mockGlobal().get(
			`${API_ROOT}/repos/${ORGANIZATION_NAME}/repo-test/issues/1/events?per_page=100`,
		...
		fetchMock
			.mockGlobal()
			.post(
				`${API_ROOT}/repos/${ORGANIZATION_NAME}/repo-test/issues`,
				({ options }) => {
		...
		fetchMock
			.mockGlobal()
			.get(`${API_ROOT}/orgs/${ORGANIZATION_NAME}/teams?per_page=100`, [
				{
					id: TEAM_ID,
					slug: "eslint-tsc",
				},
		...
		fetchMock
			.mockGlobal()
			.get(
				`${API_ROOT}/orgs/${ORGANIZATION_NAME}/teams/${TEAM_SLUG}/members?per_page=100`,
				[
					{
						id: 1,
						login: "user1",
	...
	describe("when an issue has a parseable title, has the release label, and has never been closed", () => {
		test("creates a new issue", async () => {
			...
			expect(issue.title).toBe(
				"Scheduled release for November 10th, 2017",
			);
			expect(
				issue.body.startsWith(
					"The next scheduled release will occur on Friday, November 10th, 2017",
				),
			).toBe(true);
		});
	});
