...
function mockAllOpenPrWithCommits(mockData = []) {
	mockData.forEach((pullRequest, index) => {
		...
		if (index !== mockData.length - 1) {
			linkHeaders.push(
				`<${API_ROOT}/repos/test/repo-test/pulls?state=open&page=${index + 2}>; rel="next"`,
			);
			linkHeaders.push(
				`<${API_ROOT}/repos/test/repo-test/pulls?state=open&page=${mockData.length}>; rel="last"`,
			);
		}

		if (index !== 0) {
			linkHeaders.push(
				`<${API_ROOT}/repos/test/repo-test/pulls?state=open&page=${index}>; rel="prev"`,
			);
			linkHeaders.push(
				`<${API_ROOT}/repos/test/repo-test/pulls?state=open&page=1>; rel="first"`,
			);
		}
...
describe("release-monitor", () => {
	...
	describe("issue labeled", () => {
		test("in post release phase then add appropriate status check to all PRs", async () => {
			...
			expect(
				fetchMock.callHistory.called(
					`${API_ROOT}/repos/test/repo-test/statuses/222`,
				),
			).toBeTruthy();
			expect(
				fetchMock.callHistory.called(
					`${API_ROOT}/repos/test/repo-test/statuses/333`,
				),
			).toBeTruthy();
			expect(
				fetchMock.callHistory.called(
					`${API_ROOT}/repos/test/repo-test/statuses/444`,
				),
			).toBeTruthy();
			expect(
				fetchMock.callHistory.called(
					`${API_ROOT}/repos/test/repo-test/statuses/555`,
				),
			).toBeTruthy();
			expect(
				fetchMock.callHistory.called(
					`${API_ROOT}/repos/test/repo-test/statuses/666`,
				),
			).toBeTruthy();
			expect(
				fetchMock.callHistory.called(
					`${API_ROOT}/repos/test/repo-test/statuses/777`,
				),
			).toBeTruthy();
		}, 10000);
				...
				fetchMock
					...
					.get(
						...
						[
							{
								html_url:
									"https://github.com/test/repo-test/issues/1",
							},
						],
					);
